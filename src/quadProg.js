...
/**
 * 
 * @param {Array.<Array.<number>>} a - matrix with
 * dimensions lda, n. 
 * @param {number} lda - lead dimension of the matrix a. 
 * @param {number} n - order of the matrix a. 
 * @param {Array.<number>} b - an array with dimension n. 
 ...
 * @param {Array.<number>} solution the sol vector. 
 * @param {number} value the crval output of the function. 
 * @param {Array.<number>} unconstrained_solution the dvec output. 
 * @param {Array.<number, number>} iterations - the iter value. 
 * @param {Array.<number>} iact - the iact values. 
 * @param {number} nact 
 * @param {Array.<number>} lagr - the lagrangian multipliers of the
 * quadratic form. 
 */
function solutionQP(error_index, solution, value, unconstrained_solution, iterations, iact, nact, lagr) {
    let message = '';
    if (error_index === 1) message = 'Constraints are inconsistent. No solution.';
    if (error_index === 2) message = 'The matrix A of the quadratic form 0.5 * x^t * A * x + b * x is not positive definite.';
 ...
 * 
 * @param {Array.<number>} dvec - an n x 1 vector d from above
...
function qpgen2(dmat, dvec, n, amat, bvec, q, meq, ierr = 0) {
     ...
     * were given  R^-1.
     * @param {Array.<Array.<number>>} work - vector with length
     ...
     */
    // integer parameters:
    let fddmat = n, fdamat = n, i, info, it1,
        iwnbv, iwrm, iwrv, iwsv, iwuv, iwzv, j, l,
        l1, nact, nvl, r, state = 1;
    // integer arrays:
    let iact = [], iter = [];
    // floating point parameters:
    let crval, gc, gs, nu, sum, t1, temp,
        tmpa, tmpb, tt, vsmall;
    ...
    let t1inf, t2min;
    // utility functions:
    const abs = Math.abs, max = Math.max,
        min = Math.min,
        sign = (a, b) => { return b >= 0 ? a : -a },
        sqrt = Math.sqrt;
    r = min(n, q);
